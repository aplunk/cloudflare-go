...
func TestDevicePostureIntegrationUpdate(t *testing.T) {
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		assert.Equal(t, "false", r.URL.Query().Get("test"))
		w.Header().Set("content-type", "application/json")
		fmt.Fprintf(w, `{
			"success": true,
			"errors": [],
	...
	actual, err := client.UpdateDevicePostureIntegration(context.Background(), testAccountID, want, false)
...
func TestDevicePostureIntegrationCreate(t *testing.T) {
	...
	actual, err := client.CreateDevicePostureIntegration(context.Background(), testAccountID, want, true)
